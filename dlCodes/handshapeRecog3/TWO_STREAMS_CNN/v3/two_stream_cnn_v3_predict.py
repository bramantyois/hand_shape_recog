...
def denormalize(image):
    image = (tf.cast(image, tf.float32) + 0.5) * 255
    image = tf.map_fn(int, image)
    return image    
...
def main():
    ...
    correctPrediction = tf.equal(prediction,tf.argmax(y_,1))
    ...
    with tf.Session() as sess:
        ...
        for i in range(totalBatch):
            ...
            pred,acc = sess.run(
                [prediction,accuracy],
                feed_dict={xColor:batchColor,xDepth:batchDepth, y_:batchLbl})
            averageAccuracy += acc/totalBatch
        ...
        #predictionResult =map(int,predictionResult)
