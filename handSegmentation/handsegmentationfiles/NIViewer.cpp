...
void NIViewer::run()
{
	...
	depthStream_.readFrame(&depthFrameRef_);
	colorStream_.readFrame(&colorFrameRef_);
	...
	if (depthFrameRef_.isValid())
	{
		...
		memcpy(depthFrame_.data, temporaryDepthValues_, temporaryDepthValuesSize_ * sizeof(int));
