...
openni::Status NIViewer::init()
{
	...
	//textureMapX_ = MIN_CHUNKS_SIZE(frameWidth_, TEXTURE_SIZE);
	//textureMapY_ = MIN_CHUNKS_SIZE(frameHeight_, TEXTURE_SIZE);
	//textureMap_ = new openni::RGB888Pixel[textureMapX_, textureMapY_];

	temporaryDepthValuesSize_ = frameWidth_*frameHeight_;
	temporaryDepthValues_ = new int[temporaryDepthValuesSize_];
	
	colorFrame_ = cv::Mat::zeros( frameHeight_, frameWidth_, CV_8UC3 );
	depthFrame_ = cv::Mat::zeros( frameHeight_, frameWidth_, CV_32SC1 );
	depthFrameEqed_ = cv::Mat::zeros(frameHeight_, frameWidth_, CV_32SC1);
...
void NIViewer::run()
{
	...
	if (depthFrameRef_.isValid())
	{
		...
		memset(temporaryDepthValues_, 0, temporaryDepthValuesSize_*sizeof(int));
		...
		memcpy(depthFrameEqed_.data, temporaryDepthValues_, temporaryDepthValuesSize_ * sizeof(int));
		
		depthFrame_.convertTo(depthFrame8_, CV_8UC1, 1.f / 256.f);
